...
for epoch in range(num_epochs):
    ...
    for idx, batch in enumerate(tqdm(tr_loader)):
        # print the parameter of sam_model.mask_decoder
        ...
        # print(threshold(upscaled_masks, 0.0, 0))
        # print(threshold(upscaled_masks, 0.0, 0).max())
        # exit()
        pred_mask = normalize(threshold(upscaled_masks, 0.0, 0))
        # pred_mask = torch.tensor((pred_mask > 0), dtype=torch.float, requires_grad=True)
        # pred_mask = threshold(upscaled_masks, 0.0, 0)
        gt_mask = mask.type(torch.float)
        loss = criterion(pred_mask, gt_mask)
        tr_loss_list.extend([loss[i].item() for i in range(loss.shape[0])])
        training_loss += loss

        # log.info("Epoch: {}, batch: {}, loss: {}".format(epoch, idx, loss.item()))
        # optimizer.zero_grad()
        # loss.backward()
